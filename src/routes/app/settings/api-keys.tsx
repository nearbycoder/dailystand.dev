...
import { BookText, ChevronDown, Copy, KeyRound, ShieldCheck, Trash2 } from "lucide-react"
...
function summarizePermissions(permissions: Record<string, string[]> | null): string {
...
function ApiKeysPage() {
	...
	const [expiresInOption, setExpiresInOption] = useState<ExpirationOptionValue>("90")
	...
	const createMutation = useMutation({
		mutationFn: async (input: { name: string; expiresInSeconds: number | null }) => {
		...
		onSuccess: (created) => {
			...
			if (typeof window !== "undefined") {
				window.sessionStorage.setItem(API_KEY_SESSION_STORAGE_KEY, created.key)
			}
	...
	const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
		...
		await createMutation.mutateAsync({ name: cleanName, expiresInSeconds })
	}
		...
		<div className="mx-auto w-full max-w-[1200px] px-4 py-5 sm:p-6">
			...
			<div className="mb-6 border-[3px] border-ds-border p-4 sm:p-6">
				...
				<p className="mt-2 text-xs text-ds-muted">
					Default scope includes profile, teams, standups, analytics, and owner member-management.
				</p>
				{createError && (
  ...
  "${apiBase}/api/public/v1/me"`}
			...
			<div className="border-[3px] border-ds-border p-4 sm:p-6">
				<div className="mb-3 flex items-center justify-between">
					<h2 className="text-sm font-extrabold tracking-widest">ACTIVE_KEYS</h2>
				...
				{apiKeysQuery.isLoading ? (
					...
					<div className="space-y-2">
						{apiKeysQuery.data.map((apiKey) => (
							<div key={apiKey.id} className="border-[2px] border-ds-muted3 p-3">
								<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
									<div className="min-w-0">
										...
										<p className="mt-1 text-[11px] text-ds-text-tertiary">
											{(apiKey.start ?? `${apiKey.prefix ?? "ds_"}***`).toUpperCase()}
										</p>
									</div>
									<button
										type="button"
								...
								<div className="mt-3 grid grid-cols-1 gap-2 text-[10px] font-bold tracking-widest text-ds-text-tertiary sm:grid-cols-3">
									<div>CREATED: {formatDateTime(apiKey.createdAt).toUpperCase()}</div>
									<div>EXPIRES: {formatDateTime(apiKey.expiresAt).toUpperCase()}</div>
