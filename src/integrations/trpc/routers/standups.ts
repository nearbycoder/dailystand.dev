...
import { db } from "@/db"
import { member, standupEntry, standupShare, team, teamMember } from "@/db/schema"
...
function groupEntriesByType<T extends { type: "completed" | "planned" | "blocker"; content: string }>(
	entries: T[],
) {
	const grouped = {
		completed: [] as string[],
...
function teamScopeCondition(teamId: string | null) {
	return teamId === null ? isNull(standupEntry.teamId) : eq(standupEntry.teamId, teamId)
}
...
function pushMarkdownSection(lines: string[], heading: string, items: string[]) {
...
async function getUserTeamIdsInOrganization(userId: string, organizationId: string) {
...
export const standupsRouter = {
	...
	upsertBatch: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const scopeIds = new Set(
				input.replaceScopes.map((scope) => (scope === null ? "__GLOBAL__" : scope)),
			)
	...
	getByDate: orgProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			if (input.teamId) {
				conditions.push(
					or(eq(standupEntry.teamId, input.teamId), isNull(standupEntry.teamId)),
				)
			}
	...
	exportRange: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const markdownLines: string[] = ["# STANDUP_EXPORT", "", ...metadataLines]
			...
			const teamFilePart = input.teamId ? input.teamId.slice(0, 8) : "all-teams"
	...
	getAnalytics: orgProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			startDateObj.setDate(startDateObj.getDate() - (input.rangeDays - 1))
			...
			const scopeMemberIds = new Set(
				selectedTeam
					? selectedTeam.teamMembers.map((teamMemberRow) => teamMemberRow.userId)
			...
			const blockerHotspots = teamStats
				...
				.sort(
					(a, b) =>
						b.blockerRate - a.blockerRate || b.blockers - a.blockers,
				)
				.slice(0, 6)

			const keywords = extractTopKeywords(entries.map((entry) => entry.content), 14)
			...
			const contributorDrilldown = Array.from(userStatsMap.values())
				.map((userStat) => ({
					...
					daysPosted: userStat.days.size,
					teams: userStat.teamIds.size,
					sampleTasks: (contributorTaskSamplesMap.get(userStat.userId) ?? []).slice(
						0,
						12,
					),
				}))
			...
			const recentWindow = Math.min(7, input.rangeDays)
			const recentSlice = dailyTrend.slice(-recentWindow)
			const previousSlice = dailyTrend.slice(-(recentWindow * 2), -recentWindow)
			const sumBy = (
	...
	getMine: orgProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const tokenByDate = new Map(activeShares.map((share) => [share.date, share.token]))
	...
	createDayShare: orgProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const existingShare = await db.query.standupShare.findFirst({
				...
				columns: { id: true, token: true, revokedAt: true },
			})

			if (existingShare && existingShare.revokedAt === null) {
				return {
