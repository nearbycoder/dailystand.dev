...
function groupEntriesByType(
	entries: Array<{ type: "completed" | "planned" | "blocker"; content: string }>,
) {
	const grouped = {
		completed: [] as string[],
		planned: [] as string[],
...
async function requireActor(
	...
	if (!verification.valid || !verification.key) {
		throw new McpError(401, "INVALID_API_KEY", "Invalid or unauthorized API key.")
	}
...
const tools = [
	...
	},
	{
		...
		handler: async ({ actor }: ToolContext, rawArgs: unknown) => {
			...
			const { organization } = await resolveOrganizationScope(actor.id, args.orgId)
	...
	},
	{
		...
		handler: async ({ actor }: ToolContext, rawArgs: unknown) => {
			...
			const teams = memberships
				...
				.filter((membershipTeam) => membershipTeam.organizationId === organization.id)
	...
	},
	{
		...
		handler: async ({ actor }: ToolContext, rawArgs: unknown) => {
			...
			const targetTeam = await assertTeamInOrganization(args.teamId, organization.id)
			const targetMember = await assertUserInOrganization(args.userId, organization.id)
	...
	},
	{
		...
		handler: async ({ actor }: ToolContext, rawArgs: unknown) => {
			...
			return {
				...
				breakdown: {
					completed: entries.filter((entry) => entry.type === "completed").length,
	...
	},
	{
		...
		handler: async ({ actor }: ToolContext, rawArgs: unknown) => {
			...
			return {
				organization,
				date,
				scopes: Array.from(byScope.values()).map((scopeEntries) => ({
					teamId: scopeEntries[0]!.teamId,
					teamName: scopeEntries[0]!.team?.name ?? "GENERAL",
	...
	},
	{
		...
		handler: async ({ actor }: ToolContext, rawArgs: unknown) => {
			...
			const history = dates.slice(0, args.limit).map((date) => {
	...
	},
	{
		...
		handler: async ({ actor }: ToolContext, rawArgs: unknown) => {
			...
			const standups = Array.from(byUser.values()).map((userEntries) => {
				const selectedEntries = chooseEntriesForTeam(userEntries, targetTeam.id)
				return {
...
async function handleRpcRequest(request: Request): Promise<Response> {
	...
	try {
		...
		if (method === "notifications/initialized") {
			return isNotification ? noContentResponse() : jsonResponse(rpcSuccess(id, null))
		}
		...
		return jsonResponse(rpcError(id, -32601, `Method not found: ${method}`), 404)
	} catch (error) {
		const message =
